...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (sword_t)src1 >> BITS(imm,5,0) ); //arit shift
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT( (BITS((sword_t)src1>>BITS(imm,5,0),31,0)) , 32) );
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 | imm );
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t = C(BITS(imm,11,0)); C(BITS(imm,11,0))= src1;     R(dest) = t; );
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t t = C(BITS(imm,11,0)); C(BITS(imm,11,0))= src1 | t; R(dest) = t; );
